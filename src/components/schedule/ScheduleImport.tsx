...
const ScheduleImport = ({ scheduleId, onComplete }: ScheduleImportProps) => {
  ...
  const extractPdfText = async (file: File): Promise<string> => {
    ...
    // Configure worker source - use local worker from node_modules
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      const strings = content.items.map((item: any) => item.str).filter((s: string) => s && s.trim().length);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith('.pdf')) {
        // For PDFs, send the raw file as base64 to Gemini for better accuracy
        const arrayBuffer = await file.arrayBuffer();
      ...
      toast({
        ...
        description: `Found ${data.classes.length} classes${data.warnings?.length ? ' • ' + data.warnings.join('; ') : ''}`,
      });
    } catch (error) {
